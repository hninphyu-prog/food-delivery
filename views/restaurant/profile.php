...
function getUserDetails($pdo, $user_id) {
    $stmt = $pdo->prepare("SELECT user_id, name, email, phone, address, created_at FROM users WHERE user_id = ?");
    $stmt->execute([$user_id]);
    return $stmt->fetch(PDO::FETCH_ASSOC);
}

// Fetch restaurant details if available
function getRestaurantDetails($pdo, $user_id) {
    $stmt = $pdo->prepare("SELECT restaurant_id, name, address, phone, cuisine_type, logo, status FROM restaurants WHERE user_id = ?");
    $stmt->execute([$user_id]);
    return $stmt->fetch(PDO::FETCH_ASSOC);
}
...
$restaurant_details = getRestaurantDetails($pdo, $user_id);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['update_profile'])) {
        ...
        if (empty($name) || empty($email)) {
            ...
            try {
                $update_stmt = $pdo->prepare("UPDATE users SET name = ?, email = ?, phone = ?, address = ? WHERE user_id = ?");
